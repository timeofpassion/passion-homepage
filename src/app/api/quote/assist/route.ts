...
// 우선순위: env override → sonnet(품질) → haiku(가용성 폴백). 앞이 404/실패면 다음으로.
const MODELS = [process.env.QUOTE_ASSIST_MODEL, "claude-sonnet-4-6", "claude-haiku-4-5-20251001"].filter(
  (m): m is string => !!m,
);
...
규칙(엄수):
- ★나라·지역 정확도(최우선): 고객이 말한 대상 시장(일본 / 대만 / 중국 / 국내)을 정확히 파악하고, 그 시장에 맞는 상품만 골라라. 고객이 "일본"이라 했으면 절대 대만·중국 상품을 넣지 마라(반대도 마찬가지). 나라를 혼동하는 것은 치명적 오류다. 요청에 나라가 명시되면 다른 나라 상품은 selected 에서 완전히 배제한다.
...
export async function POST(req: Request) {
  ...
  for (const model of MODELS) {
    ...
    try {
      ...
      // 모델 없음(404)·미지원·파싱 실패 → 다음 모델로 폴백
      console.error(`[quote/assist] model ${model} fail:`, (e as Error).message, rawText ? "| raw:" + rawText.slice(0, 300) : "");
      parsed = null;
    }
  }
  if (!parsed) {
