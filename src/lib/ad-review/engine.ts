...
[출력]
JSON만 출력. 코드펜스·설명 금지.
{"risk":"red|yellow|green","violations":[{"sentence":"원문에서 그대로 복사한 위반 문장","ho":0,"severity":"red|yellow","reason":"왜 저촉되는지 한 줄","fix":"바로 쓸 수 있는 수정 문안"}],"gray_notes":["조건을 갖추면 가능한 항목의 통과조건 안내"],"summary":"전체 한 줄 총평"}
sentence 는 반드시 입력 원문에 있는 그대로 복사한다(하이라이트에 쓰인다). 요약·변형 금지.
risk 는 violations 중 최고 등급. violations 가 비면 green.`;
...
function parseAiJson(s: string): AiResult | null {
  ...
  try {
    ...
    const risk: AiResult["risk"] =
      p.risk === "red" || p.risk === "yellow" || p.risk === "green"
        ? p.risk
...
export async function runAdCheck(input: string, media?: string): Promise<ScanResult> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (ai) return toScanResult(text, ai, media);
